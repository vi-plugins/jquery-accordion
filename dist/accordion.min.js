/******/ (function(modules) { // webpackBootstrap
/******/ 	// The module cache
/******/ 	var installedModules = {};
/******/
/******/ 	// The require function
/******/ 	function __webpack_require__(moduleId) {
/******/
/******/ 		// Check if module is in cache
/******/ 		if(installedModules[moduleId]) {
/******/ 			return installedModules[moduleId].exports;
/******/ 		}
/******/ 		// Create a new module (and put it into the cache)
/******/ 		var module = installedModules[moduleId] = {
/******/ 			i: moduleId,
/******/ 			l: false,
/******/ 			exports: {}
/******/ 		};
/******/
/******/ 		// Execute the module function
/******/ 		modules[moduleId].call(module.exports, module, module.exports, __webpack_require__);
/******/
/******/ 		// Flag the module as loaded
/******/ 		module.l = true;
/******/
/******/ 		// Return the exports of the module
/******/ 		return module.exports;
/******/ 	}
/******/
/******/
/******/ 	// expose the modules object (__webpack_modules__)
/******/ 	__webpack_require__.m = modules;
/******/
/******/ 	// expose the module cache
/******/ 	__webpack_require__.c = installedModules;
/******/
/******/ 	// identity function for calling harmony imports with the correct context
/******/ 	__webpack_require__.i = function(value) { return value; };
/******/
/******/ 	// define getter function for harmony exports
/******/ 	__webpack_require__.d = function(exports, name, getter) {
/******/ 		if(!__webpack_require__.o(exports, name)) {
/******/ 			Object.defineProperty(exports, name, {
/******/ 				configurable: false,
/******/ 				enumerable: true,
/******/ 				get: getter
/******/ 			});
/******/ 		}
/******/ 	};
/******/
/******/ 	// getDefaultExport function for compatibility with non-harmony modules
/******/ 	__webpack_require__.n = function(module) {
/******/ 		var getter = module && module.__esModule ?
/******/ 			function getDefault() { return module['default']; } :
/******/ 			function getModuleExports() { return module; };
/******/ 		__webpack_require__.d(getter, 'a', getter);
/******/ 		return getter;
/******/ 	};
/******/
/******/ 	// Object.prototype.hasOwnProperty.call
/******/ 	__webpack_require__.o = function(object, property) { return Object.prototype.hasOwnProperty.call(object, property); };
/******/
/******/ 	// __webpack_public_path__
/******/ 	__webpack_require__.p = "";
/******/
/******/ 	// Load entry module and return exports
/******/ 	return __webpack_require__(__webpack_require__.s = 6);
/******/ })
/************************************************************************/
/******/ ([
/* 0 */
/***/ (function(module, exports, __webpack_require__) {

"use strict";

var __extends = (this && this.__extends) || (function () {
    var extendStatics = Object.setPrototypeOf ||
        ({ __proto__: [] } instanceof Array && function (d, b) { d.__proto__ = b; }) ||
        function (d, b) { for (var p in b) if (b.hasOwnProperty(p)) d[p] = b[p]; };
    return function (d, b) {
        extendStatics(d, b);
        function __() { this.constructor = d; }
        d.prototype = b === null ? Object.create(b) : (__.prototype = b.prototype, new __());
    };
})();
Object.defineProperty(exports, "__esModule", { value: true });
var JQueryModuleBase = (function () {
    function JQueryModuleBase() {
    }
    return JQueryModuleBase;
}());
exports.JQueryModuleBase = JQueryModuleBase;
var JQueryPluginBase = (function (_super) {
    __extends(JQueryPluginBase, _super);
    function JQueryPluginBase(name, element, defaults, options) {
        var _this = _super.call(this) || this;
        _this.element = element;
        _this.$element = $(element);
        _this._$clone = _this.$element.clone(true);
        _this.options = $.extend(true, {}, defaults, options);
        _this.$element.on('init.' + name, function () {
            _this.init();
        });
        _this.$element.on('destroy.' + name, function () {
            _this.destroy();
        });
        return _this;
    }
    JQueryPluginBase.prototype.destroy = function () {
        this.$element.replaceWith(this._$clone);
    };
    ;
    return JQueryPluginBase;
}(JQueryModuleBase));
exports.JQueryPluginBase = JQueryPluginBase;


/***/ }),
/* 1 */
/***/ (function(module, exports, __webpack_require__) {

"use strict";

Object.defineProperty(exports, "__esModule", { value: true });
var JQueryPluginEvents = (function () {
    function JQueryPluginEvents() {
    }
    JQueryPluginEvents.wrapEvents = function ($element, eventName, fn, params) {
        if (params === void 0) { params = []; }
        var event = $.Event('before.' + eventName);
        var result;
        if (!$.isFunction(fn)) {
            throw 'Invalid function provided to wrapEvents';
        }
        $element.trigger(event, params);
        if (!event.isDefaultPrevented()) {
            result = fn();
            $element.trigger('after.' + eventName, params);
        }
        return result;
    };
    return JQueryPluginEvents;
}());
exports.default = JQueryPluginEvents;


/***/ }),
/* 2 */
/***/ (function(module, exports) {

// removed by extract-text-webpack-plugin

/***/ }),
/* 3 */
/***/ (function(module, exports, __webpack_require__) {

"use strict";

var __extends = (this && this.__extends) || (function () {
    var extendStatics = Object.setPrototypeOf ||
        ({ __proto__: [] } instanceof Array && function (d, b) { d.__proto__ = b; }) ||
        function (d, b) { for (var p in b) if (b.hasOwnProperty(p)) d[p] = b[p]; };
    return function (d, b) {
        extendStatics(d, b);
        function __() { this.constructor = d; }
        d.prototype = b === null ? Object.create(b) : (__.prototype = b.prototype, new __());
    };
})();
Object.defineProperty(exports, "__esModule", { value: true });
var jquery_base_1 = __webpack_require__(0);
var Animation = (function (_super) {
    __extends(Animation, _super);
    function Animation($element, options) {
        var _this = _super.call(this) || this;
        _this.$element = $element;
        _this.options = options;
        return _this;
    }
    Animation.prototype.init = function () {
        var _this = this;
        this.checkDataAttributes();
        this.$element.find('.viAccordion__titleLink').on('click.panel.accordion', function (e) {
            e.preventDefault();
            _this.togglePanel($(e.currentTarget));
        });
    };
    Animation.prototype.setActivePanel = function (openedPanel) {
        for (var i = 0; i < openedPanel.length; i++) {
            this.$element.find('.viAccordion__panel').eq(openedPanel[i]).addClass('viAccordion__panel--open');
        }
    };
    Animation.prototype.checkDataAttributes = function () {
        if (this.$element.attr('data-auto-close')) {
            this.options.autoClose = this.$element.data('auto-close');
        }
        if (this.$element.attr('data-open-duration')) {
            this.options.openDuration = this.$element.data('open-duration');
        }
        if (this.$element.attr('data-close-duration')) {
            this.options.closeDuration = this.$element.data('close-duration');
        }
        if (this.options.openedPanelIndex !== undefined) {
            this.setActivePanel(this.options.openedPanelIndex);
        }
    };
    Animation.prototype.togglePanel = function ($titleLink) {
        var $panel = $titleLink.closest('.viAccordion__panel');
        var $content = $panel.find('.viAccordion__content');
        if ($panel.hasClass('viAccordion__panel--open')) {
            this.hidePanel($content, $panel);
        }
        else {
            if (this.options.autoClose) {
                this.hideAllPannels($panel);
            }
            this.showPanel($content, $panel);
        }
    };
    Animation.prototype.hideAllPannels = function ($activePanel) {
        var _this = this;
        this.$element.find('.viAccordion__panel--open').each(function (index, elem) {
            var $elem = $(elem);
            _this.hidePanel($elem.find('.viAccordion__content'), $elem, $activePanel);
        });
    };
    Animation.prototype.showPanel = function ($content, $panel) {
        var _this = this;
        $panel
            .trigger('before.open.panel.accordion')
            .addClass('viAccordion__panel--opening');
        $content.slideDown(this.options.openDuration, function () {
            $panel
                .addClass('viAccordion__panel--open')
                .removeClass('viAccordion__panel--opening')
                .trigger('after.open.panel.accordion', { panel: $panel, position: _this.getPanelPosition($panel) });
            $content.removeAttr('style');
        });
    };
    Animation.prototype.getPanelPosition = function ($panel) {
        var opened = null;
        this.$element.find('.viAccordion__panel').each(function (index, elem) {
            if ($panel.is($(elem))) {
                opened = index;
                return false;
            }
        });
        return opened;
    };
    Animation.prototype.hidePanel = function ($content, $panel, $activePanel) {
        var _this = this;
        $panel
            .trigger('before.close.panel.accordion', [$activePanel || null])
            .addClass('viAccordion__panel--closing');
        $content.slideUp(this.options.closeDuration, function () {
            $panel
                .removeClass('viAccordion__panel--open')
                .removeClass('viAccordion__panel--closing')
                .trigger('after.close.panel.accordion', { panel: $panel, position: _this.getPanelPosition($panel) });
            $content.removeAttr('style');
        });
    };
    Animation.prototype.destroy = function () {
        this.$element.find('.viAccordion__titleLink').off('click.panel.accordion');
    };
    return Animation;
}(jquery_base_1.JQueryModuleBase));
exports.default = Animation;


/***/ }),
/* 4 */
/***/ (function(module, exports, __webpack_require__) {

"use strict";

var __extends = (this && this.__extends) || (function () {
    var extendStatics = Object.setPrototypeOf ||
        ({ __proto__: [] } instanceof Array && function (d, b) { d.__proto__ = b; }) ||
        function (d, b) { for (var p in b) if (b.hasOwnProperty(p)) d[p] = b[p]; };
    return function (d, b) {
        extendStatics(d, b);
        function __() { this.constructor = d; }
        d.prototype = b === null ? Object.create(b) : (__.prototype = b.prototype, new __());
    };
})();
Object.defineProperty(exports, "__esModule", { value: true });
var jquery_base_1 = __webpack_require__(0);
var jquery_events_1 = __webpack_require__(1);
var Scrolling = (function (_super) {
    __extends(Scrolling, _super);
    function Scrolling($element, options) {
        var _this = _super.call(this) || this;
        _this.$element = $element;
        _this.options = options;
        return _this;
    }
    Scrolling.prototype.init = function () {
        var _this = this;
        this.$element.children().on('before.close.panel.accordion', function (e, $activePanel) {
            e.preventDefault();
            if ($activePanel) {
                _this.calcScroll($activePanel);
            }
        });
        if (this.options.scrollOnOpen) {
            this.$element.children().on('after.open.panel.accordion', function (e, value) {
                e.preventDefault();
                var scrollingEnabled = _this.options.scrollOnOpenMaximumScreenWidth == null || Scrolling.getWindowWidth() <= _this.options.scrollOnOpenMaximumScreenWidth;
                if (value.panel && scrollingEnabled) {
                    var targetScrollTop = _this.getScrollTopPosition(value.panel);
                    _this.animateScrolling(targetScrollTop);
                }
            });
        }
    };
    Scrolling.prototype.calcScroll = function ($activePanel) {
        var openedContentHeight = 0;
        var elementsBeforeHeight = 0;
        if ($activePanel.prevAll('.viAccordion__panel--open').length > 0) {
            $activePanel.prevUntil('.viAccordion__panel--open').each(function (index, elem) {
                elementsBeforeHeight = elementsBeforeHeight + $(elem).outerHeight(true);
            });
            openedContentHeight = $activePanel.prevAll('.viAccordion__panel--open').find('.viAccordion__content').outerHeight();
        }
        var scrollTopPosition = this.getScrollTopPosition($activePanel);
        if (!this.isPanelInViewport(scrollTopPosition - openedContentHeight) || !this.isElementInViewport($activePanel)) {
            this.animateScrolling(scrollTopPosition - openedContentHeight);
        }
    };
    Scrolling.prototype.getScrollTopPosition = function ($elem) {
        var topOffsetElementHeight = 0;
        var topOffsetAdditional = 0;
        if (this.options.topOffsetSelector) {
            topOffsetElementHeight = $(this.options.topOffsetSelector).outerHeight();
        }
        if (this.options.topOffsetAdditional) {
            topOffsetAdditional = this.options.topOffsetAdditional;
        }
        return $elem.offset().top - (topOffsetElementHeight + topOffsetAdditional);
    };
    Scrolling.prototype.isPanelInViewport = function (scrollTopPosition) {
        return window.pageYOffset <= scrollTopPosition;
    };
    Scrolling.prototype.isElementInViewport = function ($elem) {
        return $elem.offset().top - window.pageYOffset <= $(window).height();
    };
    Scrolling.prototype.animateScrolling = function (scrollTo) {
        var _this = this;
        jquery_events_1.default.wrapEvents(this.$element, 'scroll.panel.accordion', function () {
            $('html, body').animate({
                scrollTop: scrollTo
            }, _this.options.duration);
        });
    };
    Scrolling.getWindowWidth = function () {
        return window.innerWidth || document.documentElement.clientWidth || document.body.clientWidth;
    };
    Scrolling.prototype.destroy = function () {
        this.$element.children().off('before.close.panel.accordion');
    };
    return Scrolling;
}(jquery_base_1.JQueryModuleBase));
exports.default = Scrolling;


/***/ }),
/* 5 */
/***/ (function(module, exports, __webpack_require__) {

"use strict";

var __extends = (this && this.__extends) || (function () {
    var extendStatics = Object.setPrototypeOf ||
        ({ __proto__: [] } instanceof Array && function (d, b) { d.__proto__ = b; }) ||
        function (d, b) { for (var p in b) if (b.hasOwnProperty(p)) d[p] = b[p]; };
    return function (d, b) {
        extendStatics(d, b);
        function __() { this.constructor = d; }
        d.prototype = b === null ? Object.create(b) : (__.prototype = b.prototype, new __());
    };
})();
Object.defineProperty(exports, "__esModule", { value: true });
var jquery_base_1 = __webpack_require__(0);
var jquery_events_1 = __webpack_require__(1);
var Animation = (function (_super) {
    __extends(Animation, _super);
    function Animation($element, options) {
        var _this = _super.call(this) || this;
        _this.$element = $element;
        _this.options = options;
        _this.tabsActive = false;
        return _this;
    }
    Animation.prototype.init = function () {
        if (this.$element.hasClass('tabbed')) {
            this.options.active = true;
        }
        if (this.options.active) {
            this.renderTabs();
            this.toggleView();
            this.eventListener();
        }
    };
    Animation.prototype.eventListener = function () {
        var _this = this;
        var resizeTimer;
        $(window).on('resize.window.tabs.accordion', function () {
            clearTimeout(resizeTimer);
            resizeTimer = setTimeout(function () {
                _this.toggleView();
            }, 250);
        });
        this.$element.find('.viAccordion__tabsTitleLink').on('click.panel.tabs.accordion', function (e) {
            e.preventDefault();
            _this.toggleTabs($(e.currentTarget));
        });
    };
    Animation.prototype.toggleView = function () {
        var _this = this;
        var tabsFitCheck = false;
        if (this.options.preventOverfulTabHeaders === true) {
            tabsFitCheck = this.fitIntoParent();
        }
        else {
            tabsFitCheck = true;
        }
        if (tabsFitCheck && $(window).outerWidth() >= this.options.minViewportWidth) {
            if (this.tabsActive === false) {
                jquery_events_1.default.wrapEvents(this.$element, 'show.tabs.accordion', function () {
                    _this.showTabs();
                    _this.tabsActive = true;
                });
            }
        }
        else {
            if (this.tabsActive === true) {
                jquery_events_1.default.wrapEvents(this.$element, 'hide.tabs.accordion', function () {
                    _this.hideTabs();
                    _this.tabsActive = false;
                });
            }
        }
    };
    Animation.prototype.renderTabs = function () {
        var _this = this;
        var insertedTabMarkup = {
            parent: '<ul class="viAccordion__tabs"></ul>',
            child: "\n\t\t\t\t<li class=\"viAccordion__tabsPanel\">\n\t\t\t\t\t<a class=\"viAccordion__tabsTitleLink\"></a>\n\t\t\t\t</li>"
        };
        this.$element.prepend(insertedTabMarkup.parent);
        this.$element.find('.viAccordion__titleLink').each(function (index, elem) {
            _this.$element.find('.viAccordion__tabs').append(insertedTabMarkup.child);
            if ($(elem).parents('.viAccordion__panel').is('[class*=--open]')) {
                _this.$element.find('.viAccordion__tabsPanel').eq(index).addClass('viAccordion__tabsPanel--open');
            }
            _this.$element.find('.viAccordion__tabsTitleLink').eq(index).append($(elem).html());
        });
    };
    Animation.prototype.toggleTabs = function ($elem) {
        var activeTabIndex = $elem.parents('.viAccordion__tabsPanel').index();
        this.$element.find('.viAccordion__tabsPanel').removeClass('viAccordion__tabsPanel--open');
        this.$element.find('.accordion__panel').each(function (index, elem) {
            var opened = false;
            if ($(elem).hasClass('accordion__panel--open')) {
                opened = true;
                $(elem).removeClass('accordion__panel--open');
            }
            if (opened) {
                $(elem).trigger('after.close.panel.accordion', { panel: $(event.target).parent(), position: index });
            }
        });
        this.$element.find('.viAccordion__panel').removeClass('viAccordion__panel--open');
        $elem.parents('.viAccordion__tabsPanel').addClass('viAccordion__tabsPanel--open');
        this.$element.find('.viAccordion__panel').eq(activeTabIndex)
            .addClass('viAccordion__panel--open')
            .trigger('after.open.panel.accordion', { panel: $(event.target).parent(), position: activeTabIndex });
    };
    Animation.prototype.fitIntoParent = function () {
        var contentWidth = this.$element.innerWidth();
        var tabsWidth = 0;
        var fits = true;
        this.$element.find('.viAccordion__tabsPanel').each(function (index, elem) {
            tabsWidth = tabsWidth + $(elem).outerWidth();
            if (tabsWidth >= contentWidth) {
                fits = false;
                return false;
            }
        });
        return fits;
    };
    Animation.prototype.showTabs = function () {
        var $accordionOpenedPanels = this.$element.find('.viAccordion__panel--open');
        var activePanelIndex = $accordionOpenedPanels.index() - 1;
        if ($accordionOpenedPanels.length > 1) {
            $accordionOpenedPanels.slice(1).removeClass('viAccordion__panel--open');
        }
        else if ($accordionOpenedPanels.length < 1) {
            activePanelIndex = 0;
            this.$element.find('.viAccordion__panel').eq(activePanelIndex)
                .addClass('viAccordion__panel--open');
        }
        this.$element.find('.viAccordion__tabsPanel').removeClass('viAccordion__tabsPanel--open');
        this.$element.find('.viAccordion__tabsPanel').eq(activePanelIndex).addClass('viAccordion__tabsPanel--open');
        this.$element.addClass('tabs');
        this.$element.find('.viAccordion__panel').find('.viAccordion__title').css('display', 'none');
    };
    Animation.prototype.hideTabs = function () {
        this.$element.removeClass('tabs');
        this.$element.find('.viAccordion__panel').find('.viAccordion__title').css('display', 'block');
    };
    Animation.prototype.destroy = function () {
        this.$element.find('.viAccordion__tabsTitleLink').off('click.panel.tabs.accordion');
    };
    return Animation;
}(jquery_base_1.JQueryModuleBase));
exports.default = Animation;


/***/ }),
/* 6 */
/***/ (function(module, exports, __webpack_require__) {

"use strict";

var __extends = (this && this.__extends) || (function () {
    var extendStatics = Object.setPrototypeOf ||
        ({ __proto__: [] } instanceof Array && function (d, b) { d.__proto__ = b; }) ||
        function (d, b) { for (var p in b) if (b.hasOwnProperty(p)) d[p] = b[p]; };
    return function (d, b) {
        extendStatics(d, b);
        function __() { this.constructor = d; }
        d.prototype = b === null ? Object.create(b) : (__.prototype = b.prototype, new __());
    };
})();
Object.defineProperty(exports, "__esModule", { value: true });
var jquery_base_1 = __webpack_require__(0);
var jquery_events_1 = __webpack_require__(1);
var Animation_1 = __webpack_require__(3);
var Scrolling_1 = __webpack_require__(4);
var Tabbed_1 = __webpack_require__(5);
__webpack_require__(2);
(function ($, window, document) {
    var Plugin = (function (_super) {
        __extends(Plugin, _super);
        function Plugin(element, options) {
            return _super.call(this, Plugin.NAME, element, Plugin.DEFAULTS, options) || this;
        }
        Plugin.prototype.init = function () {
            var _this = this;
            jquery_events_1.default.wrapEvents(this.$element, 'init.accordion', function () { _this.initAccordion(); });
        };
        Plugin.prototype.initAccordion = function () {
            this.animation = new Animation_1.default(this.$element, this.options.animation);
            this.animation.init();
            this.scrolling = new Scrolling_1.default(this.$element, this.options.scrolling);
            this.scrolling.init();
            this.tabbed = new Tabbed_1.default(this.$element, this.options.tabbed);
            this.tabbed.init();
        };
        Plugin.prototype.destroy = function () {
            this.animation.destroy();
            this.scrolling.destroy();
            this.tabbed.destroy();
            _super.prototype.destroy.call(this);
        };
        return Plugin;
    }(jquery_base_1.JQueryPluginBase));
    Plugin.NAME = 'accordion';
    Plugin.DEFAULTS = {
        animation: {
            autoClose: true,
            openDuration: 300,
            closeDuration: 300
        },
        scrolling: {
            active: true,
            duration: 300,
            scrollOnOpen: false
        },
        tabbed: {
            active: false,
            minViewportWidth: 0,
            preventOverfulTabHeaders: true
        }
    };
    $.fn.viAccordion = function (options) {
        return this.each(function () {
            var $this = $(this);
            if (!$this.data('viAccordion')) {
                $this.data('viAccordion', new Plugin(this, options));
            }
        });
    };
})(jQuery, window, document);


/***/ })
/******/ ]);
//# sourceMappingURL=accordion.min.js.map